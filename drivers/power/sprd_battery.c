...
#define SPRDBAT_DEBUG(format, arg...) pr_info("TPLINKLOG " format, ## arg)
...
#ifdef CONFIG_DELAY_SHUTDOWN
#define BATTERY_SHUTDOWN_LOW_TEMP	-200
#define BATTERY_SHUTDOWN_HIGH_TEMP	600
...
#ifdef CONFIG_SPROCOMM_EXHIBITION_MODE
struct tplink_charger chg_setting;
static int original_rechg;
#endif
...
static int sprdbat_is_chg_timeout(void)
{
	...
	//sprdbat_data->bat_info.chg_this_timeout = 150;

	cur_time = ktime_to_timespec64(ktime_get_boottime());
	printk("[exhibition_mode]charging timer[%lld > %d]\n", cur_time.tv_sec - sprdbat_data->bat_info.chg_start_time, sprdbat_data->bat_info.chg_this_timeout);
	if (cur_time.tv_sec - sprdbat_data->bat_info.chg_start_time >
	    sprdbat_data->bat_info.chg_this_timeout) {
		printk("[exhibition_mode][%lld > %d]timeout!!\n", cur_time.tv_sec - sprdbat_data->bat_info.chg_start_time, sprdbat_data->bat_info.chg_this_timeout);
		return 1;
	} else
		return 0;
}
...
#ifdef CONFIG_SPROCOMM_EXHIBITION_MODE
printk("wangqiang chg_setting.exhibition_mode = %d\n", chg_setting.exhibition_mode);
	sprdbat_data->pdata->rechg_vol = original_rechg; // set old rechg
	if (1 == chg_setting.exhibition_mode) {
		/*set cv and voreg*/
		if (sprdbat_data->bat_info.cur_temp > 0) {
			if (sprdbat_data->bat_info.cur_temp < EXHIBITION_CHANGE_CV_TEMP) {
				cccv = EXHIBITION_NORMAL_CV;
				sprdbat_data->pdata->rechg_vol = EXHIBITION_NORMAL_VORG;
			} else if (sprdbat_data->bat_info.cur_temp < 600) {
				cccv = EXHIBITION_HIGH_TEMP_CV;
				sprdbat_data->pdata->rechg_vol = EXHIBITION_HIGH_TEMP_VORG;
			}
		}
		printk("[exhibition_mode]:cccv=%d, rech=%d, is_limit=%d\n", cccv, sprdbat_data->pdata->rechg_vol, chg_setting.is_limit_cur);

		if (1 == chg_setting.is_limit_cur) {
...
#ifdef CONFIG_SPROCOMM_EXHIBITION_MODE
static int exhibition_mode_setting(void)
{
	int bat_vol_avg;
	bat_vol_avg = sprdbat_data->bat_info.vbat_ocv;

	/*set current*/
	if (bat_vol_avg >= 4100) {
		if ((sprdbat_data->bat_info.cur_temp >= EXHIBITION_CHANGE_CV_TEMP) || (chg_setting.is_timeout == 1) || ((sprdbat_data->bat_info.cur_temp < EXHIBITION_CHANGE_CV_TEMP) && (bat_vol_avg >= EXHIBITION_NORMAL_CV)))
			chg_setting.is_limit_cur = 1;
	} else if (bat_vol_avg < 4000) {
		chg_setting.is_limit_cur = 0;
	}

	printk("[exhibition_mode]ocv:%d, temp:%d, is_timeout:%d, is_limit_cur:%d\n", 
		bat_vol_avg, sprdbat_data->bat_info.cur_temp, chg_setting.is_timeout, chg_setting.is_limit_cur);
	return 1;
}

static ssize_t show_exhibition_mode(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("%s: %d\n", __func__, chg_setting.exhibition_mode);
	return sprintf(buf, "%d\n", chg_setting.exhibition_mode);
}

static ssize_t store_exhibition_mode(
	struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoint(buf, 10, &temp) == 0) {
		if (1 == temp) {
			chg_setting.exhibition_mode = 1;
		} else {
			chg_setting.exhibition_mode = 0;
		}
	} else {
		printk("%s: format error!\n", __func__);
	}
	
	return size;
}
static DEVICE_ATTR(exhibition_mode, 0664, show_exhibition_mode, store_exhibition_mode);
#else
...
static int sprdbat_stop_charge(void)
{
#ifndef CONFIG_SPROCOMM_EXHIBITION_MODE
...
static void sprdbat_chg_status_monitor(void)
{
	...
	SPRDBAT_DEBUG
		(" %s,ocv=%d, cur=%d,chg_end_vol_l=%d,chg_end_cur=%d,rechg_vol=%d\n",
	__func__, sprdbat_data->bat_info.vbat_ocv,
	sprdbat_data->bat_info.bat_current,
	sprdbat_data->pdata->chg_end_vol_l,
	sprdbat_data->pdata->chg_end_cur,sprdbat_data->pdata->rechg_vol);
...
#ifdef CONFIG_DELAY_SHUTDOWN
//printk("battery temperature = %d:%d\n", sprdbat_data->pdata->low_temp, sprdbat_data->pdata->high_temp);
	if ((sprdbat_data->bat_info.cur_temp <= sprdbat_data->pdata->low_temp)
		|| (sprdbat_data->bat_info.cur_temp >= sprdbat_data->pdata->high_temp)) {
		if (htimer_flag == 0) {
			htimer_flag = 1;
			ktime = ktime_set(30, 0);
			hrtimer_start(&shutdown_timer, ktime, HRTIMER_MODE_REL);
			SPRDBAT_DEBUG( "battery temperature  %d over temp, system will shutdown after 30s\n", sprdbat_data->bat_info.cur_temp);
		}
	}
#endif
...
#ifdef CONFIG_SPROCOMM_EXHIBITION_MODE
	chg_setting.is_timeout = sprdbat_is_chg_timeout();

	if (1 == chg_setting.exhibition_mode) {
		exhibition_mode_setting();
	}
#else
							   ...
							   *pdev)
{
...
#ifdef CONFIG_SPROCOMM_EXHIBITION_MODE
	original_rechg = pdata->rechg_vol;
#endif
...
#ifdef CONFIG_DELAY_SHUTDOWN
	if (of_property_read_u32(np, "shutdown-low-temp",
		(u32 *) (&pdata->low_temp))) {
		pdata->low_temp = BATTERY_SHUTDOWN_LOW_TEMP;
		dev_err(&pdev->dev, "parse shutdown-low-temp err, use default temp\n");
	} else {
		pdata->low_temp = pdata->low_temp - 1000;
	}
	if (of_property_read_u32(np, "shutdown-high-temp",
		(u32 *) (&pdata->high_temp))) {
		pdata->high_temp = BATTERY_SHUTDOWN_HIGH_TEMP;
		dev_err(&pdev->dev, "parse shutdown-high-temp err, use default temp\n");
	} else {
		pdata->high_temp = pdata->high_temp- 1000;
	}
#endif
	...
	if (of_property_read_u32(np, "rsense-psc5415a",
		(u32 *) (&pdata->psc5415a_rsense))) {
		//pdata->psc5415a_rsense = 0;
		dev_err(&pdev->dev, "parse psc5415a_rsense err\n");
	}
...
#ifdef CONFIG_DELAY_SHUTDOWN
enum hrtimer_restart shutdown_function(struct hrtimer *timer)
{
	SPRDBAT_DEBUG( "battery temperature too low, shutdown function!!!\n");
	orderly_poweroff(true);
...
#ifdef CONFIG_SPROCOMM_EXHIBITION_MODE
	ret = device_create_file(&(pdev->dev), &dev_attr_exhibition_mode);
#endif
...
#ifdef CONFIG_DELAY_SHUTDOWN
	hrtimer_init(&shutdown_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	shutdown_timer.function = shutdown_function;
#endif
