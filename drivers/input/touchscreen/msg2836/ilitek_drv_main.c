...
#define CONFIG_RESUME_WORK
#ifdef CONFIG_RESUME_WORK
	struct work_struct		liltek_resume_work;
	struct workqueue_struct	*liltek_resume_workqueue;
#endif
...
static void _DrvRemoveProcfsDirEntry(void)
{
    ...
    {
        //remove_proc_entry(PROC_NODE_LEATHER_SHEATH_MODE, _gProcDeviceEntry);  
        remove_proc_entry(PROC_NODE_CONTROL_FILM_MODE, _gProcDeviceEntry);  
...
void DrvTouchDeviceRegisterEarlySuspend(void)
{
	...
	adf_register_client(&_gFbNotifier);
...
#ifdef CONFIG_RESUME_WORK
	cancel_work_sync(&liltek_resume_work);
	destroy_workqueue(liltek_resume_workqueue);
#endif
...
#endif    
    g_TouchKObj = kobject_create_and_add("kobject_example",NULL);
    if (!g_TouchKObj) {
		printk(KERN_WARNING "kobject_create_and_add g_TouchKObj failed\n");
		kset_unregister(g_TouchKSet);
		g_TouchKSet = NULL;
		return -EINVAL;
    }
...
#endif
#if 1
    g_GestureKObj = kobject_create_and_add("kobject_gesture",NULL);
    if (!g_GestureKObj) {
		printk(KERN_WARNING "kobject_create_and_add g_GestureKObj failed\n");
		kset_unregister(g_GestureKSet);
		g_GestureKSet = NULL;
		return -EINVAL;
    }
...
void liltek_adf_resume_work(struct work_struct *work)
{
...
#ifdef CONFIG_RESUME_WORK
	queue_work(liltek_resume_workqueue, &liltek_resume_work);
#endif
...
unsigned char tp_proc_buffer[TP_PROC_BUFFER_MAX_SIZE]={0};
unsigned char tp_module_buffer[TP_PROC_BUFFER_MAX_SIZE]={0};
...
static ssize_t sprocomm_read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if(nMajor == 00001)
	{
		strcpy(tp_module_buffer, "Holitech");
	}
	else
	{
		strcpy(tp_module_buffer, "HLT");
	}
	strcpy(tp_proc_buffer, "MSG2836A");
	...
	ptr += sprintf(page, "%s:%s:%x.%x\n", tp_module_buffer,tp_proc_buffer,nMajor,nMinor);
...
#ifdef CONFIG_RESUME_WORK
	INIT_WORK(&liltek_resume_work, liltek_adf_resume_work);
	liltek_resume_workqueue = create_singlethread_workqueue("liltek_adf_resume_work");
	if (!liltek_resume_workqueue) {
		nRetVal = -ENODEV;
		return nRetVal;
	}
#endif
