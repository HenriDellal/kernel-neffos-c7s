...
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);   	
	return sprintf(buf, "gesture mode: %s\n",
	    cts_is_gesture_wakeup_enabled(&cts_data->cts_dev)? "Enable" : "Disable");}
...
static DEVICE_ATTR(gesture, S_IWUSR | S_IRUSR, gesture_show, gesture_store);
#endif
