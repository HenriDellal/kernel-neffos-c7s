					...
					size_t count)
{
	...
	tlsc_info("prox-para=%s!!\n", buf);
	
	ret = kstrtouint(buf, 0, &enable);
	if (ret)
		return -EINVAL;

	tpd_prox_cmd = (int)enable;
...
// static struct kobj_attribute tlsc6x_prox_enable_attr = {
    // .attr = {
	// .name = "facemode",
	// .mode = S_IRUGO | S_IWUGO,
	// },
    // .show = &tlsc6x_prox_enable_show,
    // .store = &tlsc6x_prox_enable_store,
// };

 static DEVICE_ATTR(facemode, 0664, tlsc6x_prox_enable_show, tlsc6x_prox_enable_store);
...
// {
	...
	// if (IS_ERR(sprd_tpd_class)) {
		// dev_err(&this_client->dev, "tlsc6x error::create sprd-tpd fail\n");
	// } else {
		...
		// } else {
			...
			//	dev_err("tlsc6x error::create device file fail(%s)!\n", dev_attr_proximity.attr.name);
			// }
		// }
	// }
...
int tlsc6x_i2c_read_sub(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
	...
	if (readlen > 0) {
		if (writelen > 0) {
			...
			if (ret < 0) {
				tlsc_err("[IIC]: i2c_transfer(1) error, addr= 0x%x!!\n", writebuf[0]);
				tlsc_err("[IIC]: i2c_transfer(1) error, ret=%d, rlen=%d, wlen=%d!!\n", ret, readlen,
				       writelen);
			}else {
	...
	char *buf)
{
	...
	ptr += sprintf(ptr,"%d, 0x%x\n",tpd_prox_active, tpd_prox_cmd);
#else
...
{
#ifdef TLSC_TPD_PROXIMITY
	tpd_prox_cmd = (int)buf[0];
	if (buf[0] == '0') {//ps off
		tlsc6x_prox_ctrl(0);
	} else if (buf[0] == '1') {//ps on
		tlsc6x_prox_ctrl(1);
	}
#endif
	return count;
...
//	tlsc6x_prox_cmd_path_init();
	...
	g_tp_drvdata->ps_input_dev->name = "alps_pxy";
