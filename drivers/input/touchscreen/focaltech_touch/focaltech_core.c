...
static struct wakeup_source ps_wakelock;
...
{
	//struct fts_data *data = i2c_get_clientdata(fts_i2c_client);
	struct fts_ts_platform_data *pdata = fts_data->pdata;//data-> fts_ts_platform_data;
	return sprintf(buf,"%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d\n"
		,__stringify(EV_KEY), __stringify(KEY_A),pdata ->keys[0],pdata ->keys[1],pdata ->keys[2],pdata ->keys[3]
		,__stringify(EV_KEY), __stringify(KEY_HOMEPAGE),pdata ->keys[4],pdata ->keys[5],pdata ->keys[6],pdata ->keys[7]
		,__stringify(EV_KEY), __stringify(KEY_BACK),pdata ->keys[8],pdata ->keys[9],pdata ->keys[10],pdata ->keys[11]);
}

static struct kobj_attribute virtual_keys_attr = {
    .attr = {
        .name = "keys.focaltech_ts",
        .mode = S_IRUGO,
    },
...
//	TS_DBG(">>>> begin, proximity_switch is 0x%.2X, on is 0x%.2X, INCALLING is 0x%.2X. <<<<\n\n",
	...
	if ((1 == on) && (0 == proximity_switch || 0 == INCALLING))
	{
		...
		__pm_stay_awake(&ps_wakelock);
		fts_i2c_write_reg(fts_i2c_client,0xB0, 0x01);
	...
	else if ((0 == on) && (1 == proximity_switch || 1 == INCALLING))
	{
		...
		__pm_relax(&ps_wakelock);
		fts_i2c_write_reg(fts_i2c_client,0xB0, 0x00);
...
//	TS_DBG(">>>> %s(%d), PROXIMITY_SWITCH is %d, PROXIMITY_STATE is %d <<<<\n", __FUNCTION__, on, PROXIMITY_SWITCH, PROXIMITY_STATE);

//	TS_DBG(">>>> end, proximity_switch is 0x%.2X, on is 0x%.2X, INCALLING is 0x%.2X. <<<<\n\n",
//	    PROXIMITY_SWITCH, on, INCALLING);
 ...
 static int ts_adf_event_handler(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	case DRM_MODE_DPMS_OFF:
		...
		break;
	default:
		FTS_INFO("receive adf event with error data, adf_event_data=%d",adf_event_data); 
		break;
	}
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    struct fts_ts_event *events = data->events;
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
 ...
 if (1 == PROXIMITY_SWITCH)
    {
    	fts_i2c_read_reg(fts_i2c_client, 0xB0, &reg_val);
	fts_i2c_read_reg(fts_i2c_client, 0x01, &reg_val1);
	...
	spin_lock(&proximity_state_lock);
        ...
        else if (0xE0 == reg_val1)
        {
            PROXIMITY_STATE = 0; // 0;
        }

        spin_unlock(&proximity_state_lock);
	 input_report_abs(data->ps_input_dev, ABS_DISTANCE, !PROXIMITY_STATE);
	 printk(" jed PROXIMITY_STATE = %d\n",PROXIMITY_STATE);
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    ts_data->events = (struct fts_ts_event *)kzalloc(point_num * sizeof(struct fts_ts_event), GFP_KERNEL);
    if (!ts_data->events) {
...
static ssize_t sprocomm_read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    u8 chip_id[2] = { 0 };
	u8 vendor_id = 0;
    err = fts_i2c_read_reg(fts_i2c_client, FTS_REG_CHIP_ID, &chip_id[0]);
    err = fts_i2c_read_reg(fts_i2c_client, FTS_REG_CHIP_ID2, &chip_id[1]);
	err = fts_i2c_read_reg(fts_i2c_client, FTS_REG_VENDOR_ID, &vendor_id);
	err = fts_i2c_read_reg(fts_i2c_client,FTS_REG_FW_VER,&tpver);
	err = fts_i2c_read_reg(fts_i2c_client,FTS_REG_VENDOR_ID,&csver);
	if((0x64 == chip_id[0])&&(0x26 == chip_id[1]))
	{	
		strcpy(tp_module_buffer, "YUYE");
		strcpy(tp_proc_buffer, "FT6436U");
	}
	else
	{
		strcpy(tp_module_buffer, "XXX");
		strcpy(tp_proc_buffer, "unknown");
	}
...
static int fts_register_powermanger(void)
{  int ret ;

	adf_event_block.notifier_call = ts_adf_event_handler;
	adf_event_block.priority = 1000;
	ret = adf_register_client(&adf_event_block);
	if (ret < 0)
		FTS_DEBUG("register adf notifier fail, cannot sleep when screen off");
	else
		FTS_DEBUG("register adf notifier succeed");
		
	return ret;
}

static int fts_unregister_powermanger(void)
{
...
//	fts_reset_proc(200);
    ...
    if (ret) {
     ...
     if(!ps_input_dev){
         err = -ENOMEM;
         dev_err(&client->dev, "fts error::failed to allocate ps-input device\n");
         goto err_irq_req;
     }
     fts_data->ps_input_dev = ps_input_dev;
     ...
     if(err){
         dev_err(&client->dev,"fts error::failed to register ps-input device\n");
         goto exit_ps_input_register_device_failed;
     }
         
	 wakeup_source_init(&ps_wakelock, "ps_wake_lock");
...
/* register external events */
#if 0
	ret = fts_register_ext_event_handler(ts_data, ts_ext_event_handler);
	if (ret < 0)
		FTS_ERROR("error in register external event!");
#endif

	ret = fts_register_powermanger();
	if (ret < 0)
		FTS_ERROR("error in register!");

	FTS_DEBUG("fts_ts_probe OK!\n");

	if (in_calibration()) {
		printk("[FTS] %s: probe cali_mode_tp\n",__func__);
		fts_irq_disable();
 ...
 exit_ps_input_register_device_failed:
		 ...
		 wakeup_source_trash(&ps_wakelock);
 #endif 
err_irq_req:
...
#ifdef TP_PROXIMITY_SENSOR
FTS_DEBUG(">>>>suspend PROXIMITY_SWITCH is %d, INCALLING is %d <<<<\n",  PROXIMITY_SWITCH, INCALLING);
	if (PROXIMITY_SWITCH || INCALLING)
	{
		  FTS_INFO("++++++++++++++++++++++++");
		suspend_flags = 0;		
#if FTS_GESTURE_EN
    ret = fts_gesture_suspend(ts_data->client, &mode);
    if (ret == 0) {
        ts_data->suspended = true;	  
    }
#endif
...
FTS_DEBUG(">>>>resume PROXIMITY_SWITCH is %d, INCALLING is %d <<<<\n",  PROXIMITY_SWITCH, INCALLING);
	if((PROXIMITY_SWITCH || INCALLING) && (suspend_flags == 0))
	{
		msleep(100);//wait for stable
		fts_i2c_write_reg(fts_i2c_client,0xB0, 0x01);
		 FTS_DEBUG("++++++++++++++PROXIMITY MODE ");
	}
#endif
...
static int __init fts_ts_init(void)
{
    ...
    FTS_FUNC_ENTER();
	printk("fts_ts_init enter\n");
	// if (in_calibration()) {
		// printk("[FTS] %s: probe cali_mode_tp\n",__func__);
		// if (gpio_request(145, "fts_reset")) {
