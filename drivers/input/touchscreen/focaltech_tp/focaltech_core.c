...
#define GPIO_LCD_ENN			5
#define GPIO_LCD_ENP			4
...
static ssize_t sprocomm_read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	{	
		strcpy(tp_module_buffer, "TIANYI");
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret) {
     ...
     if(err){
         dev_err(&client->dev,"tlsc6x error::failed to register ps-input device\n");
...
#endif
        gpio_request(GPIO_LCD_ENP, NULL);
       gpio_request(GPIO_LCD_ENN, NULL);
