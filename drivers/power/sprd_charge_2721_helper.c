...
{
	int cnt_adc_swp = 2, sum_adc = 0, adc_value;

	if (pbat_data->temp_support) {
		while (cnt_adc_swp--) {
			glb_adi_write(AUXAD_CTL0,
				(cnt_adc_swp & 0x01) ? AUXAD_NTCAMP_EN : 0,
				AUXAD_NTCAMP_EN);
			/* udealy(50) to ensure write register AUXAD_CTL0 success */
			udelay(50);
...
static int sprdchg_temp_vol_comp(int vol)
{
	...
	vol = vol - vol_comp +
		((vol_comp * (vol - vol_comp)) / (1800 - vol_comp));
	if (vol < 0)
		vol = 0;
