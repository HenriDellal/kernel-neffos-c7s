...
unsigned char sprdchg_psc5415E_cur2reg(uint32_t cur)
{
	unsigned char reg_val;

	if (cur < 1000)
		reg_val = 0;
	if ((cur >= 1000) && (cur < 1200))
		reg_val = 1;
	if ((cur >= 1200) && (cur < 1400))
		reg_val = 2;
	if ((cur >= 1400) && (cur < 1600))
		reg_val = 3;
	if ((cur >= 1600) && (cur < 1800))
		reg_val = 4;
	if ((cur >= 1800) && (cur < 2000))
		reg_val = 5;
	if ((cur >= 2000) && (cur < 2600))
		reg_val = 6;
	if (cur >= 2600)
		reg_val = 7;
	return reg_val;
...
unsigned char sprdchg_psc5415Z_cur2reg_56mohm(uint32_t cur)
{
	unsigned char reg_val;
	...
	if ((cur >= 950) && (cur < 1000))
		reg_val = 5;
	if ((cur >= 1000) && (cur < 1150))
		reg_val = 6;
	if (cur >= 1150)
		reg_val = 7;
	printk("%s:%d reg_val = %d\n", __FUNCTION__, __LINE__, reg_val);
	return reg_val;
}

unsigned char sprdchg_psc5415Z_cur2reg_33mohm(uint32_t cur)
{
	unsigned char reg_val;
	...
	if ((cur >= 1200) && (cur < 1400))
		reg_val = 3;
	if ((cur >= 1400) && (cur < 1500))
		reg_val = 4;
	if ((cur >= 1500) && (cur < 1800))
		reg_val = 5;
	if ((cur >= 1800) && (cur < 2000))
		reg_val = 6;
	if (cur >= 2000)
...
unsigned int sprdchg_psc5415Z_reg2cur_56mohm(unsigned char reg_val)
{
	unsigned int cur = 0;
...
unsigned int sprdchg_psc5415Z_reg2cur_33mohm(unsigned char reg_val)
{
	unsigned int cur = 0;
...
unsigned int sprdchg_psc5415E_reg2cur(unsigned char reg_val)
{
	unsigned int cur = 0;
...
{
	unsigned char reg_val = 0, vendor_id = VENDOR_FAN54015,rev_code = 0;
	...
	rev_code = fan54015_get_rev_code();
	if ((vendor_id == VENDOR_PSC5415)&&(rev_code == REV_CODE_PSC5415E))
		reg_val = sprdchg_psc5415E_cur2reg(cur);
	if ((vendor_id == VENDOR_PSC5415)&&(rev_code == REV_CODE_PSC5415Z)) {
	
		if (bat_drv_data->pdata->psc5415a_rsense == RSEN_56mohm)
			reg_val = sprdchg_psc5415Z_cur2reg_56mohm(cur);
		else 
			reg_val = sprdchg_psc5415Z_cur2reg_33mohm(cur);
	}
...
unsigned int sprdchg_fan54015_get_chgcur(void)
{
  ...
  if ((vendor_id == VENDOR_PSC5415)&&(rev_code == REV_CODE_PSC5415E))
		reg_val = sprdchg_psc5415E_cur2reg(reg_val);
	if ((vendor_id == VENDOR_PSC5415)&&(rev_code == REV_CODE_PSC5415Z)) {
		if (bat_drv_data->pdata->psc5415a_rsense == RSEN_56mohm)
			reg_val = sprdchg_psc5415Z_reg2cur_56mohm(reg_val);
		else
			reg_val = sprdchg_psc5415Z_reg2cur_33mohm(reg_val);
	}
