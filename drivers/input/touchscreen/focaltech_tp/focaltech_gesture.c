...
*****************************************************************************/
#define KEY_GESTURE_D                           KEY_POWER
...
#if FTS_GESTURE_EN
static ssize_t fts_gesture_write(struct file *filp,const char __user * buff, size_t len, loff_t * off)
{	
	s32 ret = 0;   
	unsigned char temp;	
	...
	if (ret) {		
	  ...
	  fts_gesture_data.mode = temp == '1'?1:0;		
	return len;
}
...
*****************************************************************************/
int fts_gesture_suspend(struct i2c_client *client, u8 *mode)
{
    int ret;
    int i;
    u8 state;
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    input_set_capability(input_dev, EV_KEY, KEY_GESTURE_D);
    ...
    __set_bit(KEY_GESTURE_D, input_dev->keybit);
    ...
    proc_entry = proc_create("gesture_enable", 0666, NULL, &gesture_fops);
    fts_gesture_data.mode = DISABLE;
    fts_gesture_data.active = DISABLE;
