...
struct chipone_ts_data *chipone_ts_data = NULL;//guo add
...
/*******************************/
	...
	if(PROXIMITY_SWITCH )
	{
		...
		cts_info("Proximity suspend enter!!!PROXIMITY_SWITCH = %d",PROXIMITY_SWITCH);
		return 0;
	}
#endif
/*******************************/
...
/*******************************/
#ifdef CONFIG_CTS_PROXIMITY
	cts_info("Proximity resume enter!!!PROXIMITY_SWITCH = %d",PROXIMITY_SWITCH);
	if((PROXIMITY_SWITCH ) )
	{
		msleep(100);//wait for stable
		cts_enable_proximity(&cts_data->cts_dev);
		return 0;
	}
#endif
/*******************************/
        ...
        const struct i2c_device_id *id)
{
	...
	chipone_ts_data = cts_data; //guo add
	...
	cts_info( "==register_early_suspend =\n" );	
	cts_data->pdata->early_suspend.level		= EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;	
	cts_data->pdata->early_suspend.suspend	= cts_suspend(cts_data);	
	cts_data->pdata->early_suspend.resume	= cts_resume(cts_data);	
	register_early_suspend( &cts_data->pdata->early_suspend );
#endif
	...
	ret = cts_proximity_input(cts_data->pdata);
	if (ret) {
	    cts_err("Proximity input failed %d", ret);
	    //goto err_free_irq;
	}
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND	
	unregister_early_suspend( &cts_data->pdata->early_suspend );
#endif
