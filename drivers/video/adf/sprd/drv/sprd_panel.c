...
static void panel_suspend(struct panel_device *pd)
{
    ...
    if (panel->specificode) {
        cancel_delayed_work_sync(&pd->specificode_work);
        pr_info("cancel SPECIFI_CODE work queue!\n");
    }
...
static void panel_resume(struct panel_device *pd)
{
    ...
    if (panel->specificode) {
        pr_info("schedule SPECIFI_CODE work queue!\n");
        schedule_delayed_work(&pd->specificode_work, msecs_to_jiffies
                    (panel->specificode));
    }

}

static void specificode_work_func(struct work_struct *work)
{

    int ret = 0;
    struct panel_device *pd = container_of(work, struct panel_device,
                specificode_work.work);
    struct panel_info *panel = pd->panel;

    ret = pd->ops->specificode_push(pd);
    if (ret) {
        pr_err("specificode send error\n");
        return;
    }

    schedule_delayed_work(&pd->specificode_work,
                msecs_to_jiffies(panel->specificode));
...
static struct panel_info *of_parse_panel(struct device_node *panel_node)
{
	...
	rc = of_property_read_string(panel_node, "lcd-glass", &lcd_glass);
	if (!rc)
		panel->panel_glass = lcd_glass;
	else
		panel->panel_glass = buffer;
	
	rc = of_property_read_string(panel_node, "lcd-module", &lcd_medule);
	if (!rc)
		panel->panel_module = lcd_medule;
	else
		panel->panel_module = buffer;
	
	rc = of_property_read_string(panel_node, "lcd-name", &lcd_name);
	if (!rc)
		panel->panel_name = lcd_name;
	else
		panel->panel_name = buffer;
    ...
    rc = of_property_read_u32(panel_node, "specificode", &temp);
    if (!rc)
      panel->specificode = temp;
    else
      panel->specificode = 0;
...
static int fix_lcd_proc_info(struct panel_info *panel)
{

	strcpy(lcd_module_buffer, panel->panel_module);
	strcpy(lcd_glass_buffer, panel->panel_glass); 
	strcpy(lcd_proc_buffer, panel->panel_name);
...
static ssize_t sprocomm_read_lcd_info(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	...
	ptr += sprintf(page, "%s:%s:%s\n", lcd_module_buffer,lcd_proc_buffer,lcd_glass_buffer);
...
static int sprd_panel_probe(struct platform_device *pdev)
{
	...
	for (index = 0; (index < 2) && panel_id[index]; index++) {
        ...
        INIT_DELAYED_WORK(&pd->specificode_work, specificode_work_func);
        if (panel->specificode) {
            pr_info("schedule SPECIFI_CODE work queue!\n");
            schedule_delayed_work(&pd->specificode_work, 
                        msecs_to_jiffies(panel->specificode));
