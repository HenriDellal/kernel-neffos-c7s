...
static uint32_t logo_bpix_form_uboot;
...
static int __init logo_bpix_get(char *str)
{
	int len = 0;

	if (str != NULL)
		len = kstrtou32(str, 10, &logo_bpix_form_uboot);
	pr_info("LCD bpix from uboot: %d\n", logo_bpix_form_uboot);
	return 0;
}
__setup("logo_bpix=", logo_bpix_get);
...
static void sprd_dispc_logo_proc(struct sprd_dispc *dispc)
{
	...
	if (logo_bpix_form_uboot > 16)
		logo_bpp = 4;

	/* should be rgb565 */
	logo_size = panel->width * panel->height * logo_bpp;
	...
	config->hwlayers[0].format = logo_bpp == 2 ?
		DRM_FORMAT_RGB565 : DRM_FORMAT_BGRA8888;
