...
static struct usb_request *mtp_request_new(struct usb_ep *ep, int buffer_size)
{
	...
	gfp_flag &= ~__GFP_DIRECT_RECLAIM;
	gfp_flag |= __GFP_NORETRY;
	/* now allocate buffers for the requests */
	req->buf = kmalloc(buffer_size, gfp_flag);
	if (!req->buf) {
