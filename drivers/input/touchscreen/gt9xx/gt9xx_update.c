...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    {
        GTP_ERROR("Inconsistent fw size. default size: %lu(%lu), file size: %u(%u)", sizeof(gtp_default_FW_fl), sizeof(gtp_default_FW_fl)/1024, update_msg.fw_total_len, update_msg.fw_total_len/1024);
        set_fs(update_msg.old_fs);
        _CLOSE_FILE(update_msg.file);
        return FAIL;
    }
