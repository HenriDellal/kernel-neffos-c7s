				...
				struct device_node *np)
{
	...
	if (IS_ERR(aon_apb))
		...
		regmap_read(aon_apb, 0x00F8, &chip_id);
	...
	if (sprd_corner_support) {
		...
		corner_layer_top.hwlayer_id = 6;
		corner_layer_bottom.hwlayer_id = 7;
	}
...
static void dpu_wb_flip(struct dispc_context *ctx)
{
	struct dpu_reg *reg = (struct dpu_reg *)ctx->base;
...
static int dpu_write_back_config(struct dispc_context *ctx)
{
	...
	/* there is a bug in ASIC writeback design, so disable it */
	return 0;
	...
	/* only enable writeback for AB chip */
...
enum {
	DPU_LAYER_ALPHA_PIXEL,
	DPU_LAYER_ALPHA_LAYER,
	DPU_LAYER_ALPHA_COMBO,
};
...
static u32 to_dpu_rotation(u32 angle)
{
	...
	case HAL_TRANSFORM_ROT_270:
		...
		break;
	default:
		pr_err("rotation convert unsupport angle=%d\n", angle);
		break;
	}
...
static u32 dpu_img_ctrl(u32 format, u32 blending, u32 compression, u32 rotation)
{
	...
	case HWC_BLENDING_COVERAGE:
		/*Normal mode*/
		reg_val &= (~BIT(16));
		reg_val |= DPU_LAYER_ALPHA_COMBO << 2;
		break;
		    ...
		    struct sprd_adf_hwlayer *hwlayer)
{
	...
	layer->ctrl = dpu_img_ctrl(hwlayer->format, hwlayer->blending,
		hwlayer->compression, hwlayer->rotation);
