...
static int check_acc_cali_data(void *cali_data)
{
	...
	pr_info("x_bias = %d; y_bias = %d; z_bias = %d\n",
	...
	if (abs(acc_cali_data.x_bias) > ACC_MAX_X_Y_BIAS_VALUE ||
		...
		pr_err("acc sensor cali failed! the bias is too big!\n");
		return -EINVAL;
	}
...
static void shub_save_calibration_data(struct work_struct *work)
{
	...
	if (sensor->cal_id == ANDROID_ACCELEROMETER) {
			 ...
			 uint8_t type, int golden_value)
{
	...
	if (cmd == CALIB_EN) {
		err = shub_send_command(sensor, id,
			...
			data, sizeof(data));
		pr_info("disable cali range check, err = %d\n", err);
	}
