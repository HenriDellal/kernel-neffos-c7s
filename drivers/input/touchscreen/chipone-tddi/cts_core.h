...
enum cts_firmware_cmd {
    ...
    CTS_CMD_PROXIMITY_ENABLE = 0xa0,
    CTS_CMD_PROXIMITY_DISABLE = 0xa1,
	#endif
};
...
struct chipone_ts_data {
    ...
    struct proc_dir_entry *procgs_entry;
#endif   
...
#ifdef CONFIG_CTS_PROXIMITY
extern void cts_enable_proximity(struct cts_device *cts_dev);
extern void cts_disable_proximity(struct cts_device *cts_dev);
extern bool cts_is_proximity_enabled(struct cts_device *cts_dev);
extern int cts_get_proximity_info(const struct cts_device *cts_dev, void *promity_info);

extern void cts_face_mode_init(void);
...
extern int cts_proximity_input(struct cts_platform_data*cts_dev);
#endif 
